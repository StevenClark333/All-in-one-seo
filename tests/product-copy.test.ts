...
test("uses beginner-safe wording for broad account and ideas surfaces", () => {
  ...
  assert.equal(
    PRODUCT_BEGINNER_COPY.issuesTypeFilterHelp,
    "Problem area, such as a missing page title or a page link that needs help.",
  );
  assert.equal(PRODUCT_BEGINNER_COPY.issuesTypeFilterLabel, "Problem area");
  assert.equal(
  ...
  assert.equal(
    PRODUCT_BEGINNER_COPY.pagesFirstActionQuiet,
    "No page needs quick care",
  );
  assert.equal(
    PRODUCT_BEGINNER_COPY.pagesFirstActionSomeProblems,
    "pages have fixable items",
  );
  assert.equal(
    PRODUCT_BEGINNER_COPY.pagesFirstActionNeedsCare,
    "pages need quick care",
  );
  assert.equal(PRODUCT_BEGINNER_COPY.pagesImportantCountLabel, "Need care");
  assert.equal(PRODUCT_BEGINNER_COPY.pagesImportantGroupLabel, "need care");
  assert.equal(
    PRODUCT_BEGINNER_COPY.pagesMetricNeedsCare,
    "Pages needing care",
  );
  assert.equal(
  ...
  assert.equal(
    ...
    "No visible link words saved yet",
  );
  assert.equal(
    PRODUCT_BEGINNER_COPY.pageDetailNoLinks,
    "No helpful links recorded yet.",
  );
  assert.equal(
    PRODUCT_BEGINNER_COPY.pagesNoQuickFix,
    "No quick fix waiting",
  );
  assert.equal(PRODUCT_BEGINNER_COPY.pagesOpenProblemsLabel, "Open problems");
  assert.equal(
    PRODUCT_BEGINNER_COPY.pagesPageChangesEmpty,
    "No important page changes yet.",
  );
  assert.equal(
    PRODUCT_BEGINNER_COPY.pagesPlanDetail,
    "Open the page list and start with pages that need the quickest care.",
  );
  assert.equal(
    PRODUCT_BEGINNER_COPY.pagesPlanIntro,
  ...
  assert.equal(
    formatProductPagesFirstAction({ pagesNeedingCare: 0, pagesWithProblems: 0 }),
    "No page needs quick care",
  );
  assert.equal(
    formatProductPagesFirstAction({ pagesNeedingCare: 0, pagesWithProblems: 4 }),
    "4 pages have fixable items",
  );
  assert.equal(
    formatProductPagesFirstAction({ pagesNeedingCare: 2, pagesWithProblems: 4 }),
    "2 pages need quick care",
  );
  assert.equal(
    formatProductPageDetailType("critical_regression"),
    "Important page change",
  );
  assert.equal(
    formatProductPageDetailType("page_noindex"),
    "Search visibility setting",
  );
  assert.equal(
    formatProductPageDetailType("missing_canonical"),
    "Preferred page setting",
  );
  ...
  assert.equal(
    formatProductProblemArea("sitemap_url_not_internally_linked:/services"),
    "Page is in the page list but needs links",
  );
  assert.equal(
    formatProductProblemArea("internally_linked_url_missing_from_sitemap"),
    "Linked page missing from page list",
  );
  assert.equal(
    formatProductProblemArea("canonical_non_200"),
    "Preferred page is not loading",
  );
  assert.equal(
    formatProductSearchIdeasContentGapDetail("wedding florist"),
    '"wedding florist" gets impressions but needs a better answer, page title, or helpful page links.',
  );
  assert.doesNotMatch(
    formatProductSearchIdeasContentGapDetail("wedding florist"),
    /internal links/i,
  ...
  assert.equal(
    PRODUCT_BEGINNER_COPY.rankCompetitorWebsiteFieldLabel,
    "Other website",
  );
  ...
  assert.equal(
    PRODUCT_BEGINNER_COPY.rankProviderFieldLabel,
    "Where this came from",
  );
  assert.equal(
    PRODUCT_BEGINNER_COPY.rankProviderPlaceholder,
  ...
  assert.equal(
    PRODUCT_BEGINNER_COPY.searchIdeasMonthlySearchesLabel,
    "Monthly searches",
  );
  assert.equal(
    PRODUCT_BEGINNER_COPY.searchIdeasMonthlySearchesMissing,
    "No monthly searches yet",
  );
  assert.equal(
