...
export async function GET(request: Request) {
  ...
  try {
    const result = await evaluateReportSchedules();
    return NextResponse.json(result);
  } catch (error) {
    logger.error("Scheduled report generation job failed.", {
      error: error instanceof Error ? error.message : "Unknown error",
    });
    await reportError({
      context: { route: "/api/jobs/generate-scheduled-reports" },
      error,
