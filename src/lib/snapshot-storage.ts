...
const localStorageRoot = ".storage/html-snapshots";
...
export async function storeHtmlSnapshot({
  ...
  const filePath = path.join(process.cwd(), localStorageRoot, key);
...
export async function readHtmlSnapshot(objectKey: string) {
  ...
  return readFile(path.join(process.cwd(), localStorageRoot, key), "utf8");
}
...
async function deleteHtmlSnapshot(objectKey: string) {
  if (objectKey.startsWith("local://")) {
    const key = objectKey.replace("local://", "");
    await rm(path.join(process.cwd(), localStorageRoot, key), {
      force: true,
    });
    return;
  }
