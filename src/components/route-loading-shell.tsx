      ...
      <div className="grid min-h-screen grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="hidden border-r border-slate-200 bg-white/95 lg:block">
          <div className="space-y-5 p-5">
            ...
            <div className="h-10 animate-pulse rounded bg-white shadow-sm" />
            {Array.from({ length: 10 }).map((_, index) => (
              <div
                key={index}
                className="h-8 animate-pulse rounded bg-slate-100/80"
              />
            ))}
          </div>
        </aside>
        <section className="px-5 py-6 sm:px-8 lg:px-10">
          <div className="border-b border-slate-200 pb-6">
            ...
            <div className="mt-3 h-9 w-80 max-w-full animate-pulse rounded bg-slate-100" />
